...
-define(distributed_pb_gpb_version, "4.10.2").

-ifndef('CLIENTINFO_PB_H').
-define('CLIENTINFO_PB_H', true).
-record('ClientInfo',
        {address = []           :: iodata() | undefined, % = 1
         port = 0               :: non_neg_integer() | undefined % = 2, 32 bits
        }).
-endif.
...
-record('SetupResponse',
        {success = false        :: boolean() | 0 | 1 | undefined, % = 1
         data = []              :: iodata() | undefined % = 2
        }).
-endif.
...
-record('CleanupInfo',
        {final = false          :: boolean() | 0 | 1 | undefined % = 1
        }).
-endif.
