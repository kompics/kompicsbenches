  ...
  loopbck uponEvent {
    case StartPartitioning(nodes) =>
      handle {
        ...
        val c = Kompics.config.copy(false).asInstanceOf[se.sics.kompics.config.Config.Impl]
        val cb = c.modify(UUID.randomUUID());
        cb.setValue("self-address", p_comp_addr)
        val cu = cb.finalise()
        val p_comp = create(classOf[PartitioningComp], KompicsInit[PartitioningComp](Some(prepare_latch), None, 1, nodes, num_keys, partition_size, true, Some(results_promise)), cu)
