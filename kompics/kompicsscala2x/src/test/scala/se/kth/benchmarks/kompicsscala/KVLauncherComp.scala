...
class KVLauncherComp(init: KompicsInit[KVLauncherComp]) extends ComponentDefinition{
  ...
  loopbck uponEvent {
    case StartPartitioning => {
      ...
      val cu = cb.finalise()
      val p_comp = create(classOf[PartitioningComp], KompicsInit[PartitioningComp](prepare_latch, None, 1, nodes.toList, num_keys, partition_size, Some(results_promise)), cu)
